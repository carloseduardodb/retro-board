  ...
  const handleRealtimeEvent = useCallback((event: RealtimeEvent) => {
    setState((prev) => {
      switch (event.type) {
        ...
        case "action_added":
          if (prev.actionCards.some((a) => a.id === event.payload.id))
            return prev;
        ...
        case "action_deleted":
          return {
            ...prev,
            actionCards: prev.actionCards.filter(
        ...
        case "suggestion_added":
          if (prev.suggestions.some((s) => s.id === event.payload.id))
            return prev;
  ...
  useEffect(() => {
    ...
    channel.on("presence", { event: "sync" }, () => {
      ...
      Object.values(presenceState).forEach((presences) => {
        (
          presences as unknown as { id: string; name: string; online_at: string }[]
        ).forEach((p) => {
          // Deduplicate by participant id — keep the latest entry
          if (
            !participantsMap.has(p.id) ||
            p.online_at > participantsMap.get(p.id)!.online_at
          ) {
