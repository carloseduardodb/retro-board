...
/**
 * Vira o board do dia em snapshot: grava o conteúdo de cada sessão sob a data
 * de referência (o dia anterior em Brasília) e limpa os boards gravados.
 *
 * Idempotente: a gravação é `ON CONFLICT DO NOTHING`, e só os boards realmente
 * inseridos são limpos — reexecutar no mesmo dia não destrói cards novos.
 */
export async function captureSnapshots(): Promise<CaptureResult> {
  ...
  try {
    ...
    if (tokens.length === 0) {
      return { captured: 0, skipped, errors };
    }
    ...
    return { captured, skipped, errors: [...errors, message] };
  }
}

/**
 * Data de referência mais recente já gravada, ou `null` se não houver nenhuma.
 * É o que diz se a virada da meia-noite aconteceu.
 */
export async function lastCapturedDate(): Promise<string | null> {
  const supabase = createServiceClient();
  ...
  if (error) {
    throw new Error(`Erro ao ler último snapshot: ${error.message}`);
  }
